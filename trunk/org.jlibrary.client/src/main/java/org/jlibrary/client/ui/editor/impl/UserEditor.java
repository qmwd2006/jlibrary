 ...
 * @author martin
 *
 * Editor to open users. It has only the metadata tab
 */
public class UserEditor extends GenericEditor {

	public static final String EDITOR_ID = "org.jlibrary.client.ui.editor.impl.UserEditor";
	
	private UserMetadataFormPage userPage;
	...
	protected MetadataFormPage createMetadataPage() {
		...
		userPage = new UserMetadataFormPage(this,Messages.getMessage(PROPERTIES));
		return userPage;
	}
	...
	{
		setModel(((UserEditorInput)getEditorInput()).getUser());
	}
	...
	public String getTitle()
	{
		try
		{
			...
			if (user.getName().equals(User.ADMIN_NAME))
				return Messages.getMessage(user.getName());
			else
				return ((User)getModel()).getName();
		}
		catch (Exception e)
		{
