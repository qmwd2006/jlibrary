 ...
 * @author martin
 *
 * <p>This action will be called when the user wants to create
 * a new bookmark</p>
 */
public class NewBookmarkAction extends SelectionDispatchAction {

	static Logger logger = LoggerFactory.getLogger(NewBookmarkAction.class);
	
	/**
	 * Constructor
	 */
	public NewBookmarkAction(IWorkbenchSite site) {
		
		super(site);

		setText(Messages.getMessage("item_new_bookmark"));
		setToolTipText(Messages.getMessage("tooltip_new_bookmark"));
	...
	public void run(Object[] elements) {
		...
		NewBookmarkWizard nbw = new NewBookmarkWizard(Bookmark.FAVORITE);
		...
		if (wd.getReturnCode() == IDialogConstants.OK_ID) {
			...
			bookmark.setType(Bookmark.FAVORITE);
			...
			try {
				...
				//Replace server "parent" by "client" parent
				bookmark.setParent(parent);
				if (parent != null) {
