 ...
 * @author martin
 */
public class GroupDragListener implements DragSourceListener {

	private TableViewer viewer;

	public GroupDragListener(TableViewer viewer) {
	...
	public void dragStart(DragSourceEvent event) {
		...
		if (selection.length > 0) {
			...
			for (int i = 0; i < selection.length; i++) {
				members[i] = (Group)selection[i].getData();
			}
			event.doit = true;
	...
	public void dragSetData (DragSourceEvent event) {
		...
		event.data = DNDItems.GROUPS_VIEW;	
	}
