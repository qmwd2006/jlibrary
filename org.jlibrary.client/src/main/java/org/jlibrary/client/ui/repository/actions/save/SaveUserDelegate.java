 ...
 *
 */
public class SaveUserDelegate extends SavingDelegate {

	static Logger logger = LoggerFactory.getLogger(SaveUserDelegate.class);
	...
	public IStatus doSave(IProgressMonitor monitor,
	    ...
	    Repository repository = RepositoryRegistry.
	    		getInstance().getRepository(user.getRepository());
		...
		try {
			...
			UserProperties userProperties = user.dumpProperties();
			...
			monitor.subTask(Messages.getAndParseValue("job_save_resource", "%1",
							user.getName()));
			user = service.updateUser(ticket, userProperties);
