...
import org.jlibrary.client.ui.security.wizard.NewGroupWizard;
 ...
 * a new group
 */
public class NewGroupAction extends SelectionDispatchAction {

	static Logger logger = LoggerFactory.getLogger(NewGroupAction.class);
	 ...
	 * Constructor
	 */
	public NewGroupAction(IWorkbenchSite site) {
		
		super(site);
		this.site = site;
		setText(Messages.getMessage("item_new_group"));
		setToolTipText(Messages.getMessage("tooltip_new_group"));
		setImageDescriptor(SharedImages.getImageDescriptor(
									SharedImages.IMAGE_NEW_GROUP));
		setDisabledImageDescriptor(SharedImages.getImageDescriptor(
				SharedImages.IMAGE_NEW_GROUP_DISABLED));
	}
	...
	public void run() {
		...
		NewGroupWizard ngw = new NewGroupWizard(repository);
	    WizardDialog wd = new WizardDialog(site.getShell(),ngw)
	    {
			protected Control createDialogArea(Composite parent)
			{
				Control control = super.createDialogArea(parent);
				getShell().setImage(SharedImages.getImage(
						SharedImages.IMAGE_GROUP));
				return control;
			}
	    };
	    wd.open();
		...
		if (wd.getReturnCode() == IDialogConstants.OK_ID) {
			Group group = ngw.getNewGroup();
			GroupsView.groupCreated(group);
